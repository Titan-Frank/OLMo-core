...
Usage (single node):
  python scripts/gpu/tokenize_dolma3_parallel.py \
...
# Per-worker global tokenizer (loaded once per worker process, not per file)
_worker_tokenizer: Optional[object] = None
_worker_tokenizer_name: Optional[str] = None


def _worker_init(tokenizer_name: str):
    """Load tokenizer once per worker and ignore SIGINT."""
...
def _tokenize_shard_with_global_tokenizer(jsonl_path: Path, npy_path: Path, eos_token_id: int) -> int:
    """Like tokenize_shard_to_npy but uses the per-worker global tokenizer."""
    all_token_ids: List[int] = []
    total_docs = 0
...
def _tokenize_wrapper(args_tuple):
    ...
    jsonl_path, npy_path, _tokenizer_name, eos_token_id = args_tuple
    try:
        n_tokens = _tokenize_shard_with_global_tokenizer(jsonl_path, npy_path, eos_token_id)
...
def main():
    ...
    if args.workers is not None:
        ...
        num_workers = min(len(my_files), max(1, (os.cpu_count() or 8) // 2))
    ...
    work_items = [(p, n, args.tokenizer, args.eos_token_id)
                  for p, n in my_files if not _npy_is_valid(n)]
    if not work_items:
        print("All assigned files already tokenized!")
        return
    ...
    if num_workers > 1:
        # Single long-lived Pool — workers load tokenizer once in _worker_init
        with mp.Pool(processes=num_workers,
                     initializer=_worker_init,
                     initargs=(args.tokenizer,)) as pool:
            # imap_unordered with chunksize for better throughput
            # Use a generous per-file timeout to detect hung/stuck workers
            PER_FILE_TIMEOUT = 1800  # 30 min per file
            for result in pool.imap_unordered(_tokenize_wrapper, work_items, chunksize=4):
                npy_path, n_tokens, error = result
                total_done += 1
                if error:
                    total_errors += 1
                ...
                if total_done % 50 == 0 or total_done == len(work_items):
                    elapsed = time.time() - start_time
                    rate = total_done / elapsed if elapsed > 0 else 0
                    eta_s = (len(work_items) - total_done) / rate if rate > 0 else 0
                    print(f"  Progress: {total_done}/{len(work_items)} done, "
                          f"{rate:.1f} files/s, "
        ...
        for item in work_items:
            npy_path, n_tokens, error = _tokenize_wrapper(item)
            total_done += 1
            if error:
                total_errors += 1
                print(f"  ERROR {npy_path.name}: {error}")
